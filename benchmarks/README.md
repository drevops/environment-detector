...
1. **Container probing ran per stack.** The detection loop tested every container-family stack, so a native-host detection re-ran the same container probe up to three times. Two changes remove that: DDEV and Lando are now identified by their own tooling's marker, so they never probe; and `Container::isContainer()` memoises its result for the run (cleared on `Environment::reset()`), so the one remaining generic-container probe is computed once. `benchDetectDrupalNative` sits ~19% below the old per-stack probing locally, and more on Linux, where the containerisation check is costlier. The single generic probe that remains is inherent to detecting an unlabelled container.
